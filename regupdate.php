...
///girls sports
	...
	{
		$shortre=clean($_POST['Shotputg']);}
		if(isset($_POST['4x100MtsRelay']))
	{
		$shortr=clean($_POST['4x100MtsRelay']);}


		$sports_women="$gTennicoit,$gthrowball,$pthrowball,$gchess,$gathletics100,$gathletics400,$glongjump,$shortrelay,$shortre,$shortr";
...
$hospitality=clean($_POST['hospitality']);
	...
	$query="update student set fname='".$fname."',lname='".$lname."',gender='".$gender."',phoneno='".$phno."',email='".$email."',state='".$state."',city='".$city."',college='".$college."',branch='".$branch."',literary_events='".$literary."',technical_events='".$technical."',sports_boys_events='".$sports_men."',sports_girls_events='".$sports_women."',fine_art_events='".$finearts."',cultural_solo_events='".$cultural_solo."',cultural_group_events='".$cultural_group."',accomodation='".$hospitality."',transport='".$transport."',date=now() where regdno='".$cid."' "; 

$result=@mysql_query($query);  
//Check whether the query was successful or not
	if($result) {
		session_write_close();
		header("location: register-update-success.php");
		exit();
	}else {
		die("Query failed");
	}
